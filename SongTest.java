	...
	public static void main (String[] args) //Εισαγωγή κύριας συνάρτησης
	{
		...
		while (true) //Εκτέλεση όσο η συνθήκη είναι αληθής
		{
			...
			if (menu==1) //Εκτέλεση πρώτης επιλογής από το Menu
			{
				...
				{
					if (sing[i]!=null) //Εκτύπωση μόνο για τα πέντε κελιά που έχουν λάβει τιμές
					{
			...
			if (menu==2) //Εκτέλεση δεύτερης επιλογής από το Menu
			{
				...
				{
					if (song[i]!=null) //Ανάγνωση μόνο για τα έξι κελιά που έχουν λάβει τιμές
					{
						if (sing[0].equals(song[i].getSinger())) //Αν ο τραγουδιστής του τραγουδιού του εκάστοτε κελιού είναι ο Frank Sinatra
						{
				...
				for (int i=0;i<song.length;i++)
				{
					if (song[i]!=null) //Εκτύπωση μόνο για τα έξι κελιά που έχουν λάβει τιμές
					{
						if (sing[0].equals(song[i].getSinger())) //Αν ο τραγουδιστής του τραγουδιού του εκάστοτε κελιού είναι ο Frank Sinatra
						{
							System.out.println(song[i]);		 //τότε το τραγούδι εκτυπώνεται σύμφωνα με τη μέθοδο toString της κλάσης Song
						}
					}
				}
				System.out.println("***********************************************************************");
